...
- HotSpot的算法实现
    ...
    - 枚举根节点- 在OopMap的协助下，HotSpot可以快速且准确地完成GC Roots枚举
        - 可作为GC Roots的节点主要在全局性的引用（例如常量或类静态属性）与执行上下文（例如栈帧中的局部变量表）中
        ...
        - GC停顿(Stop The World) 在可达性分析时对象的引用关系不能改变
...
-  Full GC (Ergonomics) 这里full gc的reason是Ergonomics，是因为开启了 UseAdaptiveSizePolicy，jvm自己进行自适应调整引发的full gc。
